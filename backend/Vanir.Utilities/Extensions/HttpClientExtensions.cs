...
namespace Vanir.Utilities.Extensions
{
    ...
    {
        public static async Task<TResult> PostAsync<TResult>(this HttpClient client, string url, HttpContent content, string oauthHeader = null)
        {
            if (!string.IsNullOrEmpty(oauthHeader))
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", oauthHeader.Split(' ')[1]);
